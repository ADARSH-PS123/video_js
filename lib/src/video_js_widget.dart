...
class VideoJsWidgetState extends State<VideoJsWidget> {
  ...
  void initState() {
    ...
    if (kIsWeb) {
      ...
      final data = ViewFactoryPlugin.platform.registerViewFactory(
          elementId: elementId,
          ...
          videoJsOptions: getVideoJsOptions(
              widget.videoJsController.videoJsOptions, widget.sources));
      return data;
    }
  }

  /// Get video initial options as a json
  Map<String, dynamic> getVideoJsOptions(
      VideoJsOptions? videoJsOptions, List<Source> sources) {
    if (videoJsOptions != null) {
      final data = videoJsOptions.toJson();
